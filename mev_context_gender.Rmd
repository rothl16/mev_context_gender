...
cross_tab <- table(df$care_0_work_1, df$Gen)
...
short_work <- means_work %>%
  ...
  filter(grepl("^(core_|value_|warmth_|competene_|pay_|cooperation_)", rowname) & 
         !grepl("_diff$", rowname)) %>%
  mutate(
    sort_key = case_when(
...
#ggsave("combined_plot_work.png", combined_plot_work, width = 12, height = 8, dpi = 300)
```
...
#ggsave("combined_plot_care.png", combined_plot_care, width = 12, height = 8, dpi = 300)
```
...
########################
#apply function for restructuring and plotting data
plot_cp_work <- plot_violin_with_means_negative(df_plot, c("cooperation_lo", "cooperation_he"), "cooperation")
